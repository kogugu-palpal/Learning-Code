...
class GreetingBot():
    ...
    def greet(self):
        ...
        if key in self.imageByTime:
            self.image_label.config(image=self.imageByTime[key])
            self.image_label.image = self.imageByTime[key]  # Keep a reference

    def load_image(self, *args):
        script_dir = os.path.dirname(os.path.abspath(__file__))
        image_keys = ["sun", "afternoon", "moon"]
        image_files = ["sun.png", "afternoon.png", "moon.png"]
        for key, filename in zip(image_keys, image_files):
